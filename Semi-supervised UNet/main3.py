...
def train(train_loader, valid_loader, unlabeled_train_df, unlabeled_valid_loader, alpha, input_model, output_model, figpath):
  ...
  #scheduler = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=0.008, total_steps=2025)
  #scheduler = optim.lr_scheduler.PolynomialLR(optimizer, total_iters = 300, power = 1)
  #scheduler = optim.lr_scheduler.ExponentialLR(optimizer, gamma = 0.9)
  #scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max = 200, eta_min = 1e-3)
  ...
  for epoch in range(1, EPOCHS+1):
    ...
    #train_loader = prepare_data_epoch(train_df)
    unlabeled_train_loader = select_random_images(unlabeled_train_df, 86)
...
def compute_metrics_test(data_set, type, model):
  ...
  with torch.no_grad():
    ...
    if type == 'labeled':
      ...
      for idx, (labeled_image, labeled_mask) in enumerate(data_set):
        # Compute the predictions
        labeled_pred_mask, _, _ = model(images=labeled_image.to(DEVICE).unsqueeze(0))

        # Move the mask tensor to the same device as the pred_mask tensor
        labeled_mask = labeled_mask.to(labeled_pred_mask.device)
      
        # Compute the IoU for this image
        iou_value = iou(labeled_pred_mask.squeeze(0), labeled_mask, type)
      ...
      for idx, (unlabeled_image1, unlabeled_image2, unlabeled_image3, unlabeled_image4) in enumerate(data_set):
        # Compute the predictions
        # N.B. CHECK THIS!!!
        unlabeled_pred_mask1, _, _ = model(images=unlabeled_image1.to(DEVICE).unsqueeze(0))
        unlabeled_pred_mask2, _, _ = model(images=unlabeled_image2.to(DEVICE).unsqueeze(0))
        unlabeled_pred_mask3, _, _ = model(images=unlabeled_image3.to(DEVICE).unsqueeze(0))
        unlabeled_pred_mask4, _, _ = model(images=unlabeled_image4.to(DEVICE).unsqueeze(0))
        unlabeled_pred_mask2 = unlabeled_pred_mask2.to(unlabeled_pred_mask1.device)
        unlabeled_pred_mask3 = unlabeled_pred_mask3.to(unlabeled_pred_mask1.device)
        unlabeled_pred_mask4 = unlabeled_pred_mask4.to(unlabeled_pred_mask1.device)

        # Compute the IoU for the unlabeled images
        iou_value1 = iou(unlabeled_pred_mask1.squeeze(0), unlabeled_pred_mask2.squeeze(0), type)
        iou_value2 = iou(unlabeled_pred_mask1.squeeze(0), unlabeled_pred_mask3.squeeze(0), type)
        iou_value3 = iou(unlabeled_pred_mask1.squeeze(0), unlabeled_pred_mask4.squeeze(0), type)
        
        iou_value = (iou_value1 + iou_value2 + iou_value3) / 3
...
def evaluate(train_set, valid_set, test_set, train_unlabeled_set, valid_unlabeled_set, test_unlabeled_set, saved_model):
  ...
  return train_iou, valid_iou, test_iou, train_unlabeled_iou, valid_unlabeled_iou, test_unlabeled_iou
