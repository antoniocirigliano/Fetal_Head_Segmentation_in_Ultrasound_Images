...
def train(train_loader, valid_loader, unlabeled_train_df, unlabeled_valid_loader, alpha, beta, input_model, output_model, figpath):

  model = SegmentationModel().to(DEVICE)

  if input_model is not None:
    # Carica lo stato del modello ignorando i parametri mancanti
    model.load_state_dict(torch.load(input_model), strict=False)
  ...
  def iter_dataloader(data_loader, unlabeled_loader, model, training):
    ...
    for (labeled_images, masks, labels), (unlabeled_images1, unlabeled_images2) in tqdm(zip(data_loader, unlabeled_loader),  total=len(unlabeled_loader)):
      ...
      labels = labels.to(DEVICE) if labels is not None else None  # Aggiunta per le etichette di classificazione
      ...
      _, labeled_loss, unlabeled_loss, _, classification_loss = model(labeled_images, masks, unlabeled_images1, unlabeled_images2, labels)
      ...
      if classification_loss is not None:
        loss += beta * classification_loss  # Aggiunta per la loss di classificazione

      if labeled_loss is not None or unlabeled_loss is not None or classification_loss is not None:
        total_loss += loss.item() 

      if training == True and (labeled_loss is not None or unlabeled_loss is not None or classification_loss is not None):
        loss.backward()
        optimizer.step()
    
    avg_loss = total_loss / (len(data_loader) + alpha * len(unlabeled_loader) + beta * len(data_loader))
  ...
  for epoch in range(1, EPOCHS+1):
    ...
    unlabeled_train_loader = select_random_images(unlabeled_train_df, 205)
...
def compute_metrics(data_set, type, model):
  ...
  with torch.no_grad():
    ...
    if type == 'labeled':
      ...
      for idx, (image, mask, label) in enumerate(data_set):

        # print(label.shape)
        # print(label)
        
        # Compute the predictions
        label_tensor = torch.tensor(label.unsqueeze(0), dtype=torch.float32)

        # print(label_tensor)
        # print(label_tensor.shape)

        pred_mask, _, _, class_probs, _= model(image.to(DEVICE).unsqueeze(0), None, None, None, label_tensor.to(DEVICE))
        ...
        labels.append(label_tensor.squeeze())
        predictions.append(class_probs.squeeze().cpu())

        # print(labels)
        # print(predictions)
      
      labels = torch.cat([label.unsqueeze(0) if label.ndimension() == 0 else label for label in labels], dim=0)
      predictions = torch.cat([pred.unsqueeze(0) if pred.ndimension() == 0 else pred for pred in predictions], dim=0)
      ...
      precision_pos, recall_pos, precision_neg, recall_neg, accuracy = binary_classification_metrics(predictions, labels)
    
    else :
      
      for idx, (image1, image2) in enumerate(data_set):
        # Compute the predictions
        pred_mask1, _, _, _, _ = model(image1.to(DEVICE).unsqueeze(0), None, None, None, None)
        pred_mask2, _, _, _, _ = model(image2.to(DEVICE).unsqueeze(0), None, None, None, None)
  ...
  if type == 'labeled':
    ...
    print(f"The Precision (head) across all {len(data_set)} images in the labeled set is {precision_pos}")
    print(f"The Recall (head) across all {len(data_set)} images in the labeled set is {recall_pos}")
    print(f"The Precision (no head) across all {len(data_set)} images in the labeled set is {precision_neg}")
    print(f"The Recall (no head) across all {len(data_set)} images in the labeled set is {recall_neg}")
    print(f"The Accuracy across all {len(data_set)} images in the labeled set is {accuracy}")
  else:
